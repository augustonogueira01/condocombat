...
class TestCreate:
    async def test_creates(
        self, repo: RivalidadeRepository, session: MagicMock
    ) -> None:
        r = _make()
...
class TestGetByApartamento:
    async def test_returns_rivalidades_do_apartamento(
        self, repo: RivalidadeRepository, session: MagicMock
    ) -> None:
    ...
    async def test_returns_empty_when_nenhuma(
        self, repo: RivalidadeRepository, session: MagicMock
    ) -> None:
...
class TestTopRivalidades:
    async def test_returns_sorted_by_nivel(
        self, repo: RivalidadeRepository, session: MagicMock
    ) -> None:
    ...
    async def test_limita_quantidade(
        self, repo: RivalidadeRepository, session: MagicMock
    ) -> None:
        session.execute.return_value.scalars.return_value.all.return_value = [
            _make(id=i) for i in range(5)
        ]
